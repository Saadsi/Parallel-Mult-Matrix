...
// Вспомогательная функция для освобождения памяти матриц
...
int main() {
    ...
    int numThreads = 0; // Количество ядер для OpenMP

    if (mode == 2) {
        cout << "Введите количество ядер (потоков) для OpenMP: ";
        cin >> numThreads;
        omp_set_num_threads(numThreads); // Установка количества ядер
    }
